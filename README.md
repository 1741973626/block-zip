...
把方块变种聚合成一个「方块集合」的 Fabric 客户端模组。

## 支持的版本

| 游戏版本 | 源码 | 下载 | 环境要求 |
| --- | --- | --- | --- |
| **Minecraft 26.3** | 本分支（`main`） | [Releases](https://github.com/1741973626/block-zip/releases) 里的 `block_zip-1.0.0.jar` | Fabric Loader 0.19+ / Fabric API 0.161+ / **Java 25** |
| **Minecraft 1.21.1** | [`1.21.1`](https://github.com/1741973626/block-zip/tree/1.21.1) 分支 | `block_zip-1.0.0+1.21.1.jar` | Fabric Loader 0.16+ / Fabric API 0.116.17+1.21.1 / **Java 21** |

两版功能与操作完全一致，但 API 与工具链差别很大，所以各自独立成一棵源码树
（1.21.1 用的是 Loom 1.7.4 + Gradle 8.10.2 + Mojang 官方映射，详见该分支的 README）。
...
* 实测方式有两种，都是真机跑出来的：
  ...
  2. **在 HMCL 启动的真实 FO 实例里手工验证**：按住 `` ` `` 展开「铜格栅」集合 →
     ...
     真的换成了「锈蚀的铜格栅」。
* **1.21.1 版**同样跑了完整自检（`BLOCKZIP_SELFTEST=1` 进游戏）：Mixin 确认生效、变种分组一致
  （铜块 8 / 切制铜 24，共 72 组）、世界与容器界面两种模式下的「按住展开 / 滚轮选择 / 松开取出 /
  鼠标不在格子上就不展开 / 长按不闪」全部通过，面板外观经截图与 26.3 版逐项比对一致。
