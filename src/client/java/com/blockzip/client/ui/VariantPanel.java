...
public final class VariantPanel {
	...
	private void toggleForScreen(AbstractContainerScreen<?> containerScreen) {
		...
		openWith(hovered.getItem(), hovered, player.getInventory().selected);
	}
	...
	public void toggleForHeldItem(Minecraft minecraft) {
		...
		this.screen = null;
		openWith(minecraft.player.getInventory().getSelected(), null,
	...
	/**
	 * 界面里的按键。1.21.1 的 Screen 收到的是 GLFW 整数，不是 KeyEvent。
	 *
	 * @param keyCode  GLFW 按键码（{@link InputConstants}）
	 * @param scanCode 平台扫描码
	 */
	public boolean onKeyPressed(AbstractContainerScreen<?> containerScreen, int keyCode, int scanCode) {
		// 打开键：和世界里一样是“按住”——按下时展开，中途的自动重复忽略，松开才取出。
		if (BlockZipClient.OPEN_VARIANTS.matches(keyCode, scanCode)) {
			if (!this.openKeyHeld) {
	...
	public boolean onMouseReleased(AbstractContainerScreen<?> containerScreen, int button) {
		if (this.swallowNextRelease) {
			this.swallowNextRelease = false;
			debug("swallowed release button=" + button
	 ...
	 * 面板范围内的鼠标按下。
	 *
	 * <p>1.21.1 的 {@code Screen#mouseClicked} 没有修饰键参数，Shift 由调用方
	 * （{@code Screen#hasShiftDown()}）判断后传进来。</p>
	 */
	public boolean onMouseClicked(AbstractContainerScreen<?> containerScreen, double mouseX, double mouseY,
			int button, boolean fullStack) {
		...
		if (inside(mouseX, mouseY)) {
			...
			if (index >= 0) {
				...
				apply(index, true, fullStack);
				// 已经取出来了，松手时不用再取一次
				this.selectionChanged = false;
			}

			debug("panel click button=" + button + " index=" + index
					+ " carried=" + containerScreen.getMenu().getCarried());
	...
	public void tickInWorld(Minecraft minecraft) {
		...
		this.hotbarIndex = minecraft.player.getInventory().selected;
		...
		boolean up = isKeyDown(InputConstants.KEY_UP);
		boolean down = isKeyDown(InputConstants.KEY_DOWN);
		boolean enter = isKeyDown(InputConstants.KEY_RETURN)
				|| isKeyDown(InputConstants.KEY_NUMPADENTER);
		...
		for (int i = 0; i < this.numberDown.length; i++) {
			boolean pressed = isKeyDown(InputConstants.KEY_1 + i);
	...
	/** 1.21.1 的 {@code InputConstants.isKeyDown} 要显式传窗口句柄。 */
	...
	public void renderInScreen(AbstractContainerScreen<?> containerScreen, GuiGraphics graphics, int mouseX, int mouseY) {
		if (!this.open || this.screen != containerScreen) {
			return;
		}
	...
	public void renderInHud(GuiGraphics graphics) {
		if (!this.open || this.screen != null) {
			return;
		}
	...
	/**
	 * 1.21.1 的 {@code blitSprite} 没有带颜色（tint）的重载，要淡出只能临时改 shader color；
	 * 画完立刻 flush，保证这一层真的用当前 alpha 画出去，再把颜色还原，不影响后面的绘制。
	 */
	private static void blitFaded(GuiGraphics graphics, ResourceLocation sprite, int spriteX, int spriteY,
			int spriteWidth, int spriteHeight, int argb) {
		float alpha = (argb >>> 24) / 255.0F;
		graphics.setColor(1.0F, 1.0F, 1.0F, alpha);
		graphics.blitSprite(sprite, spriteX, spriteY, spriteWidth, spriteHeight);
		graphics.flush();
	...
	private void render(GuiGraphics graphics, int mouseX, int mouseY) {
		...
		graphics.drawString(font, this.title, this.x + 5, this.y + 4, TITLE_COLOR, false);
		...
		for (int i = 0; i < cells; i++) {
			...
			graphics.blitSprite(SLOT_SPRITE, cellX, cellTop + i * CELL, CELL, CELL);
		}
		...
		if (selectedVisible) {
			...
			graphics.blitSprite(HOTBAR_SELECTION_SPRITE, frameX, frameY, FRAME_W, FRAME_H);
			...
			if (this.selectionPulseAt > 0L && elapsed >= 0L && elapsed < PULSE_MS) {
				...
				blitFaded(graphics, HOTBAR_SELECTION_SPRITE, frameX, frameY, FRAME_W, FRAME_H, (alpha << 24) | 0xFFFFFF);
			}
		}
		...
		if (labelIndex >= 0 && labelIndex < this.items.size()) {
			...
			graphics.drawString(font, name, textX, this.y + 1, 0xFFFFFFFF, true);
		}
		...
		graphics.drawString(font, hint, hintX, hintY, 0xFFD8D8D8, false);
	}
	...
	private void layoutForScreen(AbstractContainerScreen<?> containerScreen) {
		...
		if (anchor == null) {
			...
			if (minecraft.player != null) {
				int hotbar = this.hotbarIndex >= 0 ? this.hotbarIndex : minecraft.player.getInventory().selected;
	...
	private void apply(int index, boolean fromClick, boolean fullStack) {
		...
		if (player.isCreative()) {
			...
			if (dest == null || dest.index < 1) {
				int hotbar = this.hotbarIndex >= 0 ? this.hotbarIndex : player.getInventory().selected;
		...
		// 1.21.1：handleContainerInput -> handleInventoryMouseClick，ContainerInput -> ClickType
		minecraft.gameMode.handleInventoryMouseClick(menu.containerId, sourceId, 0, ClickType.PICKUP, player);
		minecraft.gameMode.handleInventoryMouseClick(menu.containerId, targetId, 0, ClickType.PICKUP, player);
		minecraft.gameMode.handleInventoryMouseClick(menu.containerId, sourceId, 0, ClickType.PICKUP, player);
	}
