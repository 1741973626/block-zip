...
public final class SelfTest {
	...
	private static void waitForItem(Minecraft minecraft) {
		Item copper = Items.COPPER_BLOCK.asList().get(0);
		var inventory = minecraft.player.getInventory();
		int hotbar = inventory.getSelectedSlot();

		if (inventory.getSelectedItem().getItem() != copper) {
			if (sinceGive >= 20) {
				sinceGive = 0;
	...
	private static void openWorldPanel(Minecraft minecraft) {
		...
		if (minecraft.gui.screen() != null) {
			if (phaseTicks % 40 == 0) {
				log("waiting for screen to close: " + minecraft.gui.screen());
			}
	...
	private static void repeatKeyPresses() {
		...
		KeyEvent grave = new KeyEvent(InputConstants.KEY_GRAVE, 0x29, 0);
		InputConstants.Key key = InputConstants.getKey(grave);
	...
	private static void releaseKey() {
		...
		log("before release, held = " + Minecraft.getInstance().player.getInventory().getSelectedItem());
	...
	private static void afterRelease(Minecraft minecraft) {
		...
		log("after release: panelOpen = " + VariantPanel.get().isOpen()
				+ " (expect false), held = " + minecraft.player.getInventory().getSelectedItem()
				+ " (expect minecraft:weathered_copper)");
	...
	private static void moveCursorOverSlot(Minecraft minecraft) {
		...
		if (target != null) {
			...
			minecraft.mouseHandler.onMove(window.handle(), pixelX, pixelY, 0.0, 0.0);
	...
	private static void noOpenWhenMouseOffSlot(Minecraft minecraft) {
		...
		if (phaseTicks == 5) {
			...
			minecraft.mouseHandler.onMove(window.handle(), 4.0, 4.0, 0.0, 0.0);
			return;
		}

		if (phaseTicks == 40) {
			// 按住波浪键：鼠标不在格子上 → 什么都不该展开（哪怕手上拿着一整个集合）
			VariantPanel.get().onKeyPressed(containerScreen, new KeyEvent(InputConstants.KEY_GRAVE, 0x29, 0));
			...
			VariantPanel.get().onKeyReleased(new KeyEvent(InputConstants.KEY_GRAVE, 0x29, 0));
			return;
		}
		...
		if (target != null) {
			int guiX = accessor.getLeftPos() + target.x + 8;
			int guiY = accessor.getTopPos() + target.y + 8;
			minecraft.mouseHandler.onMove(window.handle(),
					guiX * (double) window.getWidth() / window.getGuiScaledWidth(),
					guiY * (double) window.getHeight() / window.getGuiScaledHeight(), 0.0, 0.0);
		}
	...
	private static void openScreenPanel(Minecraft minecraft) {
		...
		if (screen instanceof AbstractContainerScreen<?> containerScreen) {
			...
			KeyEvent grave = new KeyEvent(InputConstants.KEY_GRAVE, 0x29, 0);
			boolean handled = VariantPanel.get().onKeyPressed(containerScreen, grave);
			log("screen initial selected = " + VariantPanel.get().selectedItemName()
					+ " (expect 鼠标下那个 = weathered_copper)");
			// 再喂一次一模一样的事件 = 模拟长按自动重复，面板不应该被关掉
			boolean repeatHandled = VariantPanel.get().onKeyPressed(containerScreen, grave);
	...
	private static void keepScreenPanel(Minecraft minecraft) {
		...
		if (phaseTicks == 60) {
			...
			if (screen instanceof AbstractContainerScreen<?> containerScreen) {
				...
				MouseButtonEvent click = new MouseButtonEvent(gx, gy, new MouseButtonInfo(0, 0));
				boolean consumed = containerScreen.mouseClicked(click, false);
				...
				boolean releaseConsumed = containerScreen.mouseReleased(click);
				...
				for (int b : new int[]{1, 3, 0}) {
					MouseButtonEvent again = new MouseButtonEvent(gx, gy, new MouseButtonInfo(b, 0));
					containerScreen.mouseClicked(again, false);
					containerScreen.mouseReleased(again);
				}
				...
				// Shift 点击 = 补满一整组
				MouseButtonEvent shiftClick = new MouseButtonEvent(gx, gy, new MouseButtonInfo(3, 1));
				containerScreen.mouseClicked(shiftClick, false);
				containerScreen.mouseReleased(shiftClick);
				log("shift click (button=3, shift): carried = " + containerScreen.getMenu().getCarried()
						+ " (expect 64 个 = Shift 拿一整组，且只有原版修饰位=1 才算 Shift)");
			}
		...
		boolean released = VariantPanel.get().onKeyReleased(new KeyEvent(InputConstants.KEY_GRAVE, 0x29, 0));
	...
	private static void leaveWorldPanelOpen(Minecraft minecraft) {
		...
		log("hotbar after screen-mode release = " + minecraft.player.getInventory().getSelectedItem()
				+ " (expect minecraft:exposed_copper)");
		minecraft.gui.setScreen(null);
		BlockZipClient.OPEN_VARIANTS.setDown(true);
		VariantPanel.get().toggleForHeldItem(minecraft);
		log("final world panel open = " + VariantPanel.get().isOpen()
				+ ", held = " + minecraft.player.getInventory().getSelectedItem());
		next(440);
	}
